...
import type { CLIAgentId } from './types.js';
...
class AgentConfigurationServiceImpl {
	...
	private generateClaudeCodeConfig(
		...
		const opusModel = config.modelSlots?.opus ?? 'gemini-claude-opus-4-5-thinking';
		...
		try {
			...
			if (mode === 'automatic') {
				...
				const shouldWriteJson = storageOption === 'json' || storageOption === 'both';
				...
				return {
					...
					shellConfig:
						storageOption === 'shell' || storageOption === 'both' ? shellExports : undefined,
					rawConfigs,
					instructions,
					modelsConfigured: 3,
					backupPath,
		...
		if (mode === 'automatic') {
			try {
				...
				return {
					...
					rawConfigs,
					instructions:
						'Configuration files created. Codex CLI is now configured to use the proxy.',
					modelsConfigured: 1,
					backupPath,
				};
			} catch (error) {
	...
	private generateAmpConfig(
		...
		const secretsJson = JSON.stringify({ [`apiKey@${baseURL}`]: config.apiKey }, null, 2);
		...
		if (mode === 'automatic') {
			try {
				...
				return {
					...
					rawConfigs,
					instructions: 'Configuration files created. Amp CLI is now configured to use the proxy.',
					modelsConfigured: 1,
				};
			} catch (error) {
				return {
