...
export const CLI_AGENTS: Record<CLIAgentId, CLIAgent> = {
	...
	amp: {
		...
		configType: 'both',
		binaryNames: ['amp'],
		configPaths: ['~/.config/amp/settings.json', '~/.local/share/amp/secrets.json'],
...
export function getVersionManagerPaths(binaryName: string, home: string): string[] {
	const paths: string[] = [];
	...
	const fnmPaths = [`${xdgDataHome}/fnm/node-versions`, `${home}/.fnm/node-versions`];
